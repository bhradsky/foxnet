...
map.files <- list.files(path = paste0(getwd(), "/outputs/applied_ecol_2022"), pattern="fox_density_scenarioname_Yyear") # set path and pattern as appropriate

map.files.path <- paste0(getwd(), "/outputs/applied_ecol_2022/", map.files)
...
writeRaster(smoothed.map, "outputs/applied_ecol_2022/scenarioname_Yyear_studentnumber_surname.asc", overwrite = TRUE)
