...
{
  experiment.values <- c(0.3, 0.5, 0.7)  
  ...
  NLLoadModel(paste0(foxnet.path, "/foxnet_model/foxnet_carnarvon_custombait.nlogo"),  
              nl.obj = corename)
  
  # Ensure model view is appropriate size (only relevant if gui = TRUE)
  NLCommand("set-patch-size 0.5", nl.obj = corename)
  ...
  NLCommand(
    ...
    "set region-size 0",
    "set landscape-raster \"gis_layers/carnarvon/carn_landscape.asc\"",
    "set uninhabitable-raster-value 0",
    "set second-habitat-raster-value 0",
    "set hab2:hab1 1",
    "set region-shp \"gis_layers/carnarvon/baited_core.shp\"",
    "set region2-shp \"\"",
  ...
  output.parameters <- c("year", 
                         ...
                         #"cub-fox-density",
                         #"fox-family-density",
                         #"count foxes with [collared = true]", 
                         #"count-baits-laid" 
                         )
  ...
  output$bait.efficacy <- i
  
  write.csv(output, paste0(foxnet.path, "/outputs/carnarvon_bait/Carnarvon_baited_", i, "_", corename,".csv"))
